...
def choose_v4(ipv4, gen_ip, ipv6) -> bool:
    ...
    passed_ip = []
    if not ping_n:
        logger_in.warning(f'Warn: Current ip is down. Open {check_thread} threads to test...')
...
def choose_v6(ipv6, gen_ipv6, ipv4) -> bool:
    ...
    passed_ip = []
    if not ping_6:
        logger_in.warning(f'Warn: Current ipv6 is down. Open {check_thread} threads to test...')
        for i in range(check_thread):
