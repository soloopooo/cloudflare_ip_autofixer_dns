...
"""
    ...
    !!!!!! MODIFIED by soloopooo FROM https://pypi.org/project/tcping/, add ipv6 support. 
    More info at https://github.com/zhengxiaowai/tcping, based on MIT License.
"""
...
class Ping(object):
    ...
    def ping(self, count=10, ipv6 = False):
        for n in range(1, count + 1):
            s = self._create_socket(socket.AF_INET6, socket.SOCK_STREAM) if ipv6 else self._create_socket(socket.AF_INET, socket.SOCK_STREAM)
            try:
                time.sleep(1)
...
@click.option('--ipv6','-6', default=False, help='ipv6')
@click.argument('host')
def cli(host, port, count, timeout, report,ipv6):
    ping = Ping(host, port, timeout)
    try:
        ping.ping(count,ipv6)
    except KeyboardInterrupt:
        pass
